...
def test_patch_device_invalid_json(drv):
    with pytest.raises(NetsimError, match="invalid JSON"):
        drv.patch_device("1", "not json")
...
def test_set_radio_invalid_type(drv):
    with pytest.raises(NetsimError, match="unknown radio"):
        drv.set_radio("1", "zigbee", "on")


def test_set_radio_invalid_state(drv):
    with pytest.raises(NetsimError, match="invalid enabled.*yes"):
        drv.set_radio("1", "ble", "yes")
